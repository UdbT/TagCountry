...
# Read number of point in each country
with open(os.path.join(os.getcwd(), "country-count.json"), 'r') as fp:
    country_count = json.load(fp)

# Create the color mapper
color_mapper = LogColorMapper(palette=gpalette)

# Read World shapefile
grid_fp = os.path.join(os.getcwd(), "shapefile", "ne_50m_admin_0_countries.shp")

# Read files
grid = gpd.read_file(grid_fp, driver="shapefile")
grid["geometry"] = grid["geometry"].apply(lambda x: list(x) if "Multi" in str(type(x)) else x)
grid = grid.set_index(['NAME_EN'])['geometry'].apply(pd.Series).stack().reset_index() # Expanding list to rows
grid.drop("level_1", axis=1, inplace=True)
grid.rename(columns={'NAME_EN':'name', 0:'geometry'}, inplace=True)
...
grid['amount'] = grid["name"].apply(lambda x: country_count[x] if x in country_count else 0)
...
main_plot = figure(title="World Map", plot_width=1500, plot_height=800,
...
#             maximum = max(province_paths[selected_province], key=province_paths[selected_province].get)
...
#             # Set node hover
#             node_hover_tool = HoverTool(renderers=[graph], tooltips=[("Province", "@index"), ("Degree", "@degree")])
...
# gsource.selected.on_change('indices', gcallback) # Add callback when grid is selected

# # Network Graph -------------------------------------------------
# import networkx as nx

# # Initialize side figure
# side_plot = figure(title="Travel Network", x_range=(97 ,106), y_range=(5, 21), plot_width=480, plot_height=800)

# # Plot Thailand map layout
# spglyph = side_plot.patches('x', 'y', source=gsource, fill_color=None, line_color="black", line_width=0.5, line_alpha=0.5)
# spglyph.nonselection_glyph = None
   
# with open("provinces.json", 'r') as fp:
#     provinces = json.load(fp)["provinces"]
...
#                 pathToProv = [prov for prov in pathToProv if str(prov) != 'nan']
#                 if not province in graph_dict:
#                     graph_dict[province] = nx.Graph(name=province)
#                 graph_dict[province].add_edges_from([(pathToProv[x], pathToProv[x+1]) for x in range(len(pathToProv)-1)])
#             except:
#                 pass

# # Read thai provinces coordination file
# prov_info = pd.read_csv(os.path.join(os.getcwd(), "thai-provinces", "thai-provinces_point.csv"), index_col=1)
# prov_info = prov_info.to_dict()

# # Create network glyph for each graph
# bokeh_graph = dict()
# for prov in graph_dict.keys():

#     if "Bangkok" in prov: continue

#     graph = graph_dict[prov]

#     # Add extra attributes to nodes for display in a hover
#     nx.set_node_attributes(graph, {k:v for k,v in dict(graph.degree).items()} , "degree")
#     nx.set_node_attributes(graph, {k:v for k,v in dict(graph.degree).items()} , "size")
...
#     nsource = ColumnDataSource(pd.DataFrame.from_dict({k:v for k,v in graph.nodes(data=True)},orient='index'))

#     # Color mapper
#     color_mapper = LogColorMapper(palette=npalette)

#     # Graph renderer using nx
#     ngraph = from_networkx(graph, {k:v for k, v in zip(nsource.data['index'], zip(nsource.data['lon'], nsource.data['lat']))})

#     # Style node
#     ngraph.node_renderer.data_source.data.update(nsource.data) # Tell glyph to use our source
#     ngraph.node_renderer.glyph = Circle(size='size', line_width=0.5, fill_color={'field': 'degree', 'transform': color_mapper})
...
# def bcallback():
#     # Reset main plot
#     gsource.data['amount'] = list(grid["name"].apply(lambda x: province_count[x] if x in province_count else 0))
